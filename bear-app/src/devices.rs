...
impl<T: Read> device::Device for StdinDevice<T> {
    fn ioctl(&mut self, command: u32) -> u32 {
        ...
        match self.state {
            device::GenericDeviceState::ReadyForCommand =>
                match command {
                    ...
                    Some(device::GenericDeviceCommand::Execute{ command, argument: _ }) => {
...
impl<T: Write> device::Device for StdoutDevice<T> {
    ...
    fn dma_poll(&mut self) -> Option<device::DMARequest> { None }
